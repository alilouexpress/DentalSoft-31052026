        ...
        <div className="space-y-2">
          {patientTimeline.map((log: any) => (
            <div key={log.id} className="flex items-start gap-3 p-3 bg-white rounded-lg border border-slate-200">
              <div className="mt-1.5"><div className="h-2 w-2 rounded-full bg-teal-500" /></div>
  ...
  return (
    <div className="flex-1 min-w-0 space-y-5">
      ...
      {/* ════════════════════════════════════════════════════════════
          ...
          ════════════════════════════════════════════════════════════ */}
      <div className="relative bg-gradient-to-br from-white via-white to-teal-50/30 dark:from-slate-900 dark:via-slate-900 dark:to-teal-950/20 rounded-2xl border border-slate-200/80 dark:border-slate-700/50 shadow-lg shadow-teal-500/5 overflow-hidden">
        {/* Animated gradient accent stripe */}
        <div className="h-1.5 bg-gradient-to-r from-teal-400 via-teal-500 to-emerald-500" style={{ backgroundSize: '200% 100%', animation: 'shimmer 3s ease-in-out infinite' }} />

        <div className="p-6 sm:p-7 space-y-5">
          {/* Avatar + Identity */}
          <div className="flex gap-5 sm:gap-6">
            <div className="relative group">
              <Avatar className="h-[88px] w-[88px] border-[3px] border-white dark:border-slate-700 shadow-lg shadow-teal-500/20 shrink-0 ring-2 ring-teal-500/20 transition-transform duration-300 group-hover:scale-105">
                <AvatarImage src={patient.photoUrl || undefined} />
                <AvatarFallback className="text-2xl font-black bg-gradient-to-br from-teal-500 via-teal-600 to-emerald-600 text-white">{initials}</AvatarFallback>
              </Avatar>
            ...
            <div className="flex-1 min-w-0 space-y-1.5">
              ...
              <div className="flex items-center gap-2.5 text-sm text-slate-500 dark:text-slate-400 flex-wrap">
                <span className="font-mono font-bold text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/30 px-2 py-0.5 rounded-md text-xs">{patient.patientId}</span>
                <span className="text-slate-300 dark:text-slate-600">|</span>
                <span className="font-medium">{patient.age} ans</span>
                <span className="text-slate-300 dark:text-slate-600">|</span>
                <span className="font-medium">{patient.gender === "Male" ? "Homme" : "Femme"}</span>
                {patient.phone && <><span className="text-slate-300 dark:text-slate-600">|</span><span className="flex items-center gap-1.5 font-medium"><Phone className="h-3.5 w-3.5 text-teal-500" />{patient.phone}</span></>}
              </div>
              {patient.email && (
              ...
              <div className="flex items-center gap-3 text-sm text-slate-500 dark:text-slate-400 flex-wrap pt-0.5">
                <span className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5 text-teal-500" /><span className="font-semibold text-slate-600 dark:text-slate-300">Dernière visite:</span> {patient.lastVisit ? format(new Date(patient.lastVisit), "dd/MM/yyyy") : <span className="italic text-slate-400">Aucune</span>}</span>
                <span className="text-slate-300 dark:text-slate-600">|</span>
                <span className="flex items-center gap-1.5"><CalendarCheck className="h-3.5 w-3.5 text-teal-500" /><span className="font-semibold text-slate-600 dark:text-slate-300">Prochain RDV:</span> <span className="italic text-slate-400">Aucun</span></span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <Button variant="outline" size="sm" className="h-9 gap-1.5 text-sm border-teal-200 dark:border-teal-800 text-teal-700 dark:text-teal-400 hover:bg-teal-50 dark:hover:bg-teal-950/30 cursor-pointer transition-all duration-200 hover:shadow-sm hover:-translate-y-px" onClick={() => patient.phone && window.open(`tel:${patient.phone}`)}><Phone className="h-3.5 w-3.5" /> Appeler</Button>
            <Button variant="outline" size="sm" className="h-9 gap-1.5 text-sm border-teal-200 dark:border-teal-800 text-teal-700 dark:text-teal-400 hover:bg-teal-50 dark:hover:bg-teal-950/30 cursor-pointer transition-all duration-200 hover:shadow-sm hover:-translate-y-px" onClick={() => patient.email && window.open(`mailto:${patient.email}`)}><Mail className="h-3.5 w-3.5" /> Email</Button>
            <Button size="sm" className="h-9 gap-1.5 text-sm bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 text-white cursor-pointer shadow-md shadow-teal-500/25 transition-all duration-200 hover:shadow-lg hover:shadow-teal-500/30 hover:-translate-y-px" onClick={() => setRdvOpen(true)}><CalendarPlus className="h-3.5 w-3.5" /> Nouveau RDV</Button>
          ...
          <div className={`relative rounded-xl p-4 flex items-center justify-between overflow-hidden ${hasBalance
            ...
            : "bg-gradient-to-r from-emerald-50 to-teal-50/50 dark:from-emerald-950/20 dark:to-teal-950/10 border border-emerald-200/60 dark:border-emerald-800/30"
            }`}>
        ...
        <div className="flex overflow-x-auto scrollbar-hide">
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                <Icon className={`h-4 w-4 transition-colors duration-300 ${isActive ? "text-teal-600 dark:text-teal-400" : "text-slate-400 dark:text-slate-500 group-hover:text-slate-500 dark:group-hover:text-slate-400"}`} />
                {tab.label}
                {badge && (
                ...
                <span className={`absolute bottom-0 left-2 right-2 h-[3px] rounded-full transition-all duration-300 ${isActive
                  ? "bg-gradient-to-r from-teal-500 to-emerald-500 opacity-100"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[65fr_35fr] gap-6">
        ...
        <div className="space-y-4">
          ...
          <Card className="border-slate-200/80 dark:border-slate-700/50 shadow-md shadow-teal-500/5 rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-teal-500/10">
                ...
                <div className="animate-in fade-in duration-300">
                  {activeTreatments.length === 0 ? (
                    <div>
                      ...
                      <Button size="sm" variant="outline" className="w-full h-9 gap-1.5 text-sm text-teal-700 dark:text-teal-400 border-teal-200 dark:border-teal-800 hover:bg-teal-50 dark:hover:bg-teal-950/30 cursor-pointer transition-all duration-200" onClick={() => setTab("treatment-plan")}><Plus className="h-3.5 w-3.5" /> Créer un plan de soins</Button>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {activeTreatments.slice(0, 3).map((t: any) => (
                        <div key={t.id} className="flex items-center justify-between py-2 px-2 rounded-lg border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                          ...
                          <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0 cursor-pointer hover:bg-teal-50 dark:hover:bg-teal-900/30" onClick={() => setTab("treatment-plan")} aria-label="Voir le plan de soins"><Eye className="h-3.5 w-3.5 text-slate-500 dark:text-slate-400" /></Button>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="animate-in fade-in duration-300">
                  {patientPayments.length === 0 ? (
                    <div>
                      ...
                      <Button size="sm" variant="outline" className="w-full h-9 gap-1.5 text-sm text-teal-700 dark:text-teal-400 border-teal-200 dark:border-teal-800 hover:bg-teal-50 dark:hover:bg-teal-950/30 cursor-pointer transition-all duration-200" onClick={() => { setPaymentToEdit(null); setPaymentOpen(true); }}><Plus className="h-3.5 w-3.5" /> Ajouter un paiement</Button>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2 pt-1">
                        <Button size="sm" variant="ghost" className="flex-1 h-8 text-xs font-semibold text-teal-600 dark:text-teal-400 hover:text-teal-700 hover:bg-teal-50 dark:hover:bg-teal-950/30 cursor-pointer" onClick={() => setTab("payments")}>Voir tout ({patientPayments.length})</Button>
                        <Button size="sm" variant="outline" className="h-8 gap-1 text-xs text-teal-700 dark:text-teal-400 border-teal-200 dark:border-teal-800 hover:bg-teal-50 dark:hover:bg-teal-950/30 cursor-pointer" onClick={() => { setPaymentToEdit(null); setPaymentOpen(true); }}><Plus className="h-3 w-3" /> Ajouter</Button>
                      </div>
                    </div>
                  )}
                </div>
